...
if (!params.multipleSequencingRuns && !params.pacbio){
	...
	process dada_pacBio {
		...
		dada2_filter_pacbio.r --infile ${demux} --filterDir dada2_filtered --maxEE ${params.maxEE} --truncLen ${trunc} --minLen ${params.minLen} --maxLen ${params.maxLen} --stats filter_stats.tsv --threads ${task.cpus} --verbose

		# Estimation of error models with DADA2 learnErrors
		dada2_errmodels_pacbio.r --filterDir dada2_filtered --threads ${task.cpus} > err.out

		# Denoise samples with DADA2
		dada2_denoise_pacbio.r --filterDir dada2_filtered --errModel err.rds --pool TRUE --threads ${task.cpus} --verbose > dd.out
		...
		dada2_chimrem.r --manifest ${demux} --dadaObj dd.rds --method "pooled" --allowOneOff TRUE --table feature-table.tsv --reltable rel-feature-table.tsv --repseqs sequences.fasta --stats denoise_stats.tsv --threads ${task.cpus}
